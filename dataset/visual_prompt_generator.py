...
def draw_mask(draw, bbox_coord,  segmentation_coords, color="red", width=1,   ):
    if isinstance(segmentation_coords, np.ndarray):
        height, width = segmentation_coords.shape
        mask_img = Image.fromarray(segmentation_coords.astype('uint8') * 255)
        draw.bitmap((0, 0), mask_img, fill=color)

    elif isinstance(segmentation_coords, list):
        for segment in segmentation_coords:
            coords = [(segment[i], segment[i + 1]) for i in range(0, len(segment), 2)]
            draw.polygon(coords, fill=color)

    elif segmentation_coords is None:
        coords = [
            bbox_coord[0], bbox_coord[1],
            bbox_coord[0], bbox_coord[3],
            bbox_coord[2], bbox_coord[3],
            bbox_coord[2], bbox_coord[1]
        ]
        draw.polygon([(coords[i], coords[i + 1]) for i in range(0, len(coords), 2)],
                    fill=color)
    else:
        print(f"Warning: Unexpected segmentation data type: {type(segmentation_coords)}")
