...
import re 
import json
from dataset.visual_prompt_generator import image_blending, color_pool, words_shape
import base64
...
tactile_expressions = [
"The object {preposition} {color}{shape} gives tactile feelings of?",
"The object {preposition} {color}{shape} evokes a sense of?",
"The object {preposition} {color}{shape} imparts a tactile sensation of?",
"The object {preposition} {color}{shape} conveys a touchable quality of?",
"The object {preposition} {color}{shape} communicates a palpable feeling of?",
"The object {preposition} {color}{shape} suggests a tactile experience of?",
"The object {preposition} {color}{shape} manifests a tangible sensation of?",
"The object {preposition} {color}{shape} elicits a haptic impression of?",
"The object {preposition} {color}{shape} gives rise to a tactile perception of?",
"The object {preposition} {color}{shape} induces a touch-sensitive feeling of?",
"The object {preposition} {color}{shape} brings forth a tactile awareness of?",
"The object {preposition} {color}{shape} arouses a tactile familiarity of?",
"The object {preposition} {color}{shape} renders a tactile essence of?",
"The object {preposition} {color}{shape} stimulates a touch-based sensation of?",
"The object {preposition} {color}{shape} invokes a tactile resonance of?",
"The object {preposition} {color}{shape} delivers a touch-oriented impression of?",
"The object {preposition} {color}{shape} offers a tactile nuance of?",
"The object {preposition} {color}{shape} provides a tactile sense of?",
"The object {preposition} {color}{shape} yields a touch-felt experience of?",
"The object {preposition} {color}{shape} reveals a tactile characteristic of?",
"The object {preposition} {color}{shape} bestows a tactile attribute of?",
"The object {preposition} {color}{shape} imparts a sense of tactile",
"The object {preposition} {color}{shape} stimulates tactile sensations of?",
"The object {preposition} {color}{shape} hints at a tactile experience of?",
"The object {preposition} {color}{shape} embodies a tactile quality of?",
"The object {preposition} {color}{shape} resonates with tactile feelings of?",
"The object {preposition} {color}{shape} conveys tactile impressions of?",
"The object {preposition} {color}{shape} suggests a tactile nature of?",
"The object {preposition} {color}{shape} evokes tactile attributes of?",
"The object {preposition} {color}{shape} communicates a tactile essence of?",
"The object {preposition} {color}{shape} reveals tactile characteristics of?",
"The object {preposition} {color}{shape} portrays tactile elements of?",
"The object {preposition} {color}{shape} brings to mind tactile aspects of?",
"The object {preposition} {color}{shape} offers tactile nuances of?",
"The object {preposition} {color}{shape} provides tactile insights into",
"The object {preposition} {color}{shape} captures tactile features of?",
"The object {preposition} {color}{shape} projects tactile properties of?",
"The object {preposition} {color}{shape} hints at tactile textures of?",
"The object {preposition} {color}{shape} introduces tactile dimensions of?",
"The object {preposition} {color}{shape} manifests tactile facets of?",
"The object {preposition} {color}{shape} presents tactile qualities of?",
"The object {preposition} {color}{shape} elucidates tactile attributes of?"
]
...
def vip_conv_generator(source, sampled_shapes, dataset_type, sub_type = ''):
    ...
    if dataset_type == 'refcocog':
        ...
        if sub_type == 'gpt4v':
            ...
            convs_source.append([prompt, source['answer']])
    elif dataset_type.startswith('Tac'):
        if sub_type == 'gpt4v':
            color_name, _, _ = sampled_shapes[0]
            color_string = f' {color_name}' if color_name!= None else ''
            text = f'within the{color_string} rectangle'
            for i in range(len(source['conversations'])):
                source['conversations'][i]['value'] = source['conversations'][i]['value'].replace('<bbox>', text)
            source['conversation'][0]['value'] = '<image>\n' + source['conversation'][0]['value']
            return source['conversations']
        else:
...
def create_question_prompt_tactile(line, visual_prompt_shape_choices, color_list):
    all_instance_index = [0]
    shape_color_info = get_color_shape(all_instance_index, visual_prompt_shape_choices, color_list)
    color_name, color_rgb, shape = shape_color_info[0]
    color_str = f'{color_name} ' if color_name is not None else ''
    shape_name = shape  # Get actual shape from parameters
    if shape_name == 'mask':
        preposition = 'with the'
    else:
        preposition = 'within the'

    question_template = random.choice(tactile_expressions)
    question_prompt = question_template.format(preposition=preposition, color=color_str, shape=shape_name)
    question_prompt = f'<image>\n{question_prompt}'

    # Process answer for consistency
    answer = line['answer'].strip().capitalize()
    if not answer.endswith('.'):
        answer += '.'  # Ensure proper punctuation
    ...
    return shape_color_info, conversations, line['bboxes']
...
visual_prompt_config = dict(
    ...
    TacCOCO = [ ["rectangle", "mask"], 'constant'],
) 
...
#                 # Decode base64 counts back to bytes
#                 segmentation['counts'] = base64.b64decode(segmentation['counts'].encode('utf-8'))
...
def vip_processor(source, image, image_size_anchor, data_args):
    ...
    elif dataset_type in {'TacCOCO'}:
        shape_color_info, conversation, bboxes = create_question_prompt_tactile(source, visual_prompt_shape_choices, color_list = list(color_pool.items()))
        source['bboxes'] = bboxes
    else:
        used_colors = [] 
